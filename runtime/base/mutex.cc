...
class ScopedAllMutexesLock {
 public:
  explicit ScopedAllMutexesLock(const BaseMutex* mutex) : mutex_(mutex) {
    while (!gAllMutexData->all_mutexes_guard.CompareAndSwap(0, reinterpret_cast<uintptr_t>(mutex))) {
      NanoSleep(100);
    }
  }
  ~ScopedAllMutexesLock() {
    while (!gAllMutexData->all_mutexes_guard.CompareAndSwap(reinterpret_cast<uintptr_t>(mutex_), 0)) {
      NanoSleep(100);
    }
  }
 private:
